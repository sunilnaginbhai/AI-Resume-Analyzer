    ...
    const handleAnalyze = async ({ companyName, jobTitle, jobDescription, file }: { companyName: string, jobTitle: string, jobDescription: string, file: File }) => {
        ...
        try {
            ...
            const data = {
                ...
                jobTitle,
                jobDescription,
                feedback: {}, // Will be populated as an object
            };

            setStatusText('Analyzing...');

            // FIX: Use ai.chat with a string prompt to satisfy the DeepSeek 'text' requirement
            const prompt = `${prepareInstructions({ jobTitle, jobDescription })}\n\nPlease analyze the resume for the ${jobTitle} position at ${companyName}. Return ONLY a JSON object.`;
            ...
            // Convert the string feedback into the nested object structure
            try {
                data.feedback = JSON.parse(feedbackText);
            } catch (e) {
                console.warn("AI didn't return valid JSON, using raw text", feedbackText);
            ...
            <section className="main-section">
                <div className="page-heading py-16">
                    ...
                    {isProcessing ? (
                        <>
                            <h2>{statusText}</h2>
                            <img src="/images/resume-scan.gif" className="w-full" alt="reseume-sacn"/>
                        </>
                    ) : (
